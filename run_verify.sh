...
LOG=/private/tmp/fix90/verify_results.log
: > $LOG
run() {
  echo "\n===== $1 =====" >> $LOG
  shift
...
run "FUZZ structural-inclusive" env FUZZ_SEEDS=60 FUZZ_STEPS=200 cargo test -p ironcalc_base --test fuzz_differential -- --nocapture
run "FUZZ verify" env FUZZ_SEEDS=20 FUZZ_STEPS=120 cargo test -p ironcalc_base --features recalc_verify --test fuzz_differential -- --nocapture
echo "\nALL DONE" >> $LOG
