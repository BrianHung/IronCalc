...
fn fn_duration_mduration_edge_cases() {
    ...
    model._set("A1", BOND_SETTLEMENT);
    model._set("A2", BOND_MATURITY_1DAY);
    model._set("A3", BOND_MATURITY_4Y);

    let test_cases = [
        ("B", "A1", "A2", STD_COUPON, STD_YIELD, "short_term"),
        ("C", "A1", "A3", 0.0, STD_YIELD, "zero_coupon"),
        ("D", "A1", "A3", STD_COUPON, 0.0, "zero_yield"),
        ("E", "A1", "A3", 1.0, 0.5, "high_rates"),
    ];
