...
#[test]
fn test_fn_median_even_length_datasets() {
    let mut model = new_empty_model();
...
fn test_fn_median_odd_length_datasets() {
    ...
    model._set("D6", "6.6");
    model._set("D7", "7.7");
    model._set("A3", "=MEDIAN(D1:D7)");
    model.evaluate();
    assert_eq!(model._get_text("A3"), *"4.4");
}

#[test]
fn test_fn_median_all_identical_values() {
    let mut model = new_empty_model();
...
fn test_fn_median_mixed_argument_types() {
    ...
    // Test mixing ranges, individual values, and references
    model._set("D1", "100");
    model._set("A3", "=MEDIAN(B1:B3, D1, 50)");
    model.evaluate();
    // Values: [1, 3, 5, 100, 50] sorted: [1, 3, 5, 50, 100] -> median = 5
    assert_eq!(model._get_text("A3"), *"5");
}

#[test]
fn test_fn_median_large_datasets() {
    let mut model = new_empty_model();
...
#[test]
fn test_fn_median_precision_and_large_numbers() {
    let mut model = new_empty_model();
    ...
    model._set("B2", "=MEDIAN(C1:C4)");
    model.evaluate();
    assert_eq!(model._get_text("B2"), *"2500000");

    // Test with scientific notation
    model._set("D1", "1E6");
    model._set("D2", "2E6");
    model._set("D3", "3E6");
    model._set("B3", "=MEDIAN(D1:D3)");
    model.evaluate();
    assert_eq!(model._get_text("B3"), *"2000000");
}
...
#[test]
fn test_fn_median_edge_case_zeros_and_small_numbers() {
    let mut model = new_empty_model();
