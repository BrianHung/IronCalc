...
| `dependency_graph.rs` | The graph itself: edges keyed by cell, range, and input, a banded range index (`SheetRanges`), `replace_reads`, `begin_rebuild`/`end_rebuild` (a full pass is a mark and a sweep, not a clear), `reachable_within` (the cone walk, which stops at the size its caller has already decided to fall back at), `topo_order`, `structural_edit`/`structural_move`, and `RecalcMode`. A structural edit shifts every index through `Shift`, applied field by field in `shift`, which destructures the struct so a new index cannot skip it. The two edits differ only in the `Remap` the shared `Displacement` carries — see "A move is a shift too" below. Also `SheetLayout`, the sheet numbering the stored positions are expressed in — see "Sheet numbering" below. |
...
The first is the whole-graph cycle rebuild `nodes()` + `cycle_cone` ran at the end of every traced full pass. It was 60-84% of the entire gap to `Full` on the three fallback shapes — 0.97 ms of a 1.22 ms gap on `dashboard`, 4.07 ms of 6.76 ms on `long-chain`, 0.86 ms of 1.02 ms on `spill-heavy` — and on a workbook with no cycle in it every microsecond of that produced the empty set. It is now gated on the pass's own `#CIRC!` witness: see "Cycles cost nothing when there are none" above for why the witness can be trusted in the only direction that matters.
